...
def extract_drawing(filename):
    ...
    spim_table_dic = {'Layer':'', 'Specification Thickness':'', 'Tolerance -':'', 'Tolerance +':'', '품목번호':''}
    spim_table_dic['Layer'] = 'PSR T1'
    spim_table_dic['Specification Thickness'] = next(((float(item['THICKNESS'])*1000) for item in stackup if item['ID'] == 'CAD_TOP_SOLDERMASK'),None)
    spim_table_dic['Tolerance -'] = next(((float(item['TOLERANCE'])*1000) for item in stackup if item['ID'] == 'CAD_TOP_SOLDERMASK'),None)
    spim_table_dic['Tolerance +'] = spim_table_dic['Tolerance -']

    spim_table.append(spim_table_dic)

    while ispim < half_count:

        ipoommok = 1

        spim_table_dic = {'Layer':'', 'Specification Thickness':'', 'Tolerance -':'', 'Tolerance +':'', '품목번호':''}
        spim_table_dic['Layer'] = f'L{ispim+1}'
        spim_table_dic['Specification Thickness'] = next(((float(item['THICKNESS'])*1000) for item in stackup if item['ID'] == f'CAD_LAYER_{ispim+1}'),None)
        spim_table_dic['Tolerance -'] = next(((float(item['TOLERANCE'])*1000) for item in stackup if item['ID'] == f'CAD_LAYER_{ispim+1}'),None)
        spim_table_dic['Tolerance +'] = spim_table_dic['Tolerance -']
        
        spim_table.append(spim_table_dic)

        if ispim < half_count -1:

            spim_table_dic = {'Layer':'', 'Specification Thickness':'', 'Tolerance -':'', 'Tolerance +':'', '품목번호':''}
            spim_table_dic['Layer'] = f'D{ispim+1}'
            spim_table_dic['Specification Thickness'] = next(((float(item['THICKNESS'])*1000) for item in stackup if item['ID'] == f'CAD_DIELECTRIC{ispim+1}_{ispim+2}'),None)
            spim_table_dic['Tolerance -'] = next(((float(item['TOLERANCE'])*1000) for item in stackup if item['ID'] == f'CAD_DIELECTRIC{ispim+1}_{ispim+2}'),None)
            spim_table_dic['Tolerance +'] = spim_table_dic['Tolerance -']
            spim_table_dic['품목번호'] = next((item['item_no'] for item in in_table if int(item['layer']) == (ipoommok+1)),None)
            spim_table.append(spim_table_dic)
    ...
    spim_table_dic = {'Layer':'', 'Specification Thickness':'', 'Tolerance -':'', 'Tolerance +':'', '품목번호':''}
    spim_table_dic['Layer'] = 'CORE'
    spim_table_dic['Specification Thickness'] = next(((float(item['THICKNESS'])*1000) for item in stackup if item['ID'] == f'CAD_DIELECTRIC{ispim}_{ispim+1}'),None)
    spim_table_dic['Tolerance -'] = next(((float(item['TOLERANCE'])*1000) for item in stackup if item['ID'] == f'CAD_DIELECTRIC{ispim}_{ispim+1}'),None)
    spim_table_dic['Tolerance +'] = spim_table_dic['Tolerance -']
    #이자리에 나중에 CCL...
    spim_table.append(spim_table_dic)

    while ispim < int(layer_count):
    ...
    spim_table_dic = {'Layer':'', 'Specification Thickness':'', 'Tolerance -':'', 'Tolerance +':'', '품목번호':''}
    spim_table_dic['Layer'] = 'PSR B1'
    spim_table_dic['Specification Thickness'] = next(((float(item['THICKNESS'])*1000) for item in stackup if item['ID'] == 'CAD_BOTTOM_SOLDERMASK'),None)
    spim_table_dic['Tolerance -'] = next(((float(item['TOLERANCE'])*1000) for item in stackup if item['ID'] == 'CAD_BOTTOM_SOLDERMASK'),None)
    spim_table_dic['Tolerance +'] = spim_table_dic['Tolerance -']
    spim_table.append(spim_table_dic)
    ...
    return stackup, in_table, spim_table



# 사용 예시
if __name__ == '__main__':
    data1, data2, data3 = extract_drawing('in_a.txt')  # 파일명 변경 가능
    ...
    with pd.ExcelWriter('ex_a.xlsx', engine='openpyxl') as writer:
        ...
        df3.to_excel(writer, sheet_name='spim_input', index=False)
